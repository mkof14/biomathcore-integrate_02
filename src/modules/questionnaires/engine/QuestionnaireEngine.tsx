...
export default function QuestionnaireEngine({
  ...
  const [metricMode, setMetricMode] = useState<"percent" | "score" | "level">(initialMetric);
...
<div className="mx-auto" style={{ maxWidth: 1200, padding: "24px 24px 80px 24px" }}>
        <div style={{ position: "sticky", top: 0, zIndex: 5, backdropFilter: "blur(2px)" }} className="py-3 mb-6">
          <div className="text-sm opacity-90" style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <label style={{display:"inline-flex",alignItems:"center",gap:6}}>
                <input type="radio" name="metricMode" checked={metricMode==="percent"} onChange={()=>setMetricMode("percent")} /> <span>%</span>
              </label>
              <label style={{display:"inline-flex",alignItems:"center",gap:6}}>
                <input type="radio" name="metricMode" checked={metricMode==="score"} onChange={()=>setMetricMode("score")} /> <span>Score</span>
              </label>
              <label style={{display:"inline-flex",alignItems:"center",gap:6}}>
                <input type="radio" name="metricMode" checked={metricMode==="level"} onChange={()=>setMetricMode("level")} /> <span>Level</span>
              </label>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-10">
            {modules.map((mod) => (
              <fieldset key={mod.key} style={{ border: "1px solid var(--qa-fieldset-border)", borderRadius: 12, padding: 20, background: "rgba(255,255,255,0.06)" }}>
                <legend style={{ padding: "0 8px", fontSize: "1rem", fontWeight: 600 }}>{mod.title || mod.key}</legend>
