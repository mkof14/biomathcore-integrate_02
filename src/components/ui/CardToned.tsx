...
export function CardHeader({
  ...
  return (
    <div className={cn("p-4 border-b border-neutral-800/60", className)}>
      {children}
    </div>
  );
}
...
export function CardFooter({
  ...
  return (
    <div
      className={cn(
        "p-4 border-t border-neutral-800/60 flex items-center gap-2",
        className,
      )}
    >
      {children}
...
export function Btn({
  ...
  const base =
    "inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-sm border transition";
  const variants = {
    primary: "bg-violet-600 hover:bg-violet-500 text-white border-violet-500",
    ghost:
      "bg-neutral-900/40 hover:bg-neutral-800/60 text-neutral-100 border-neutral-700/60",
  } as const;
...
export function CardSub({
  ...
  return (
    <div className={cn("text-sm text-neutral-300", className)}>{children}</div>
  );
}
