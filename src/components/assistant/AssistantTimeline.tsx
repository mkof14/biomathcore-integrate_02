...
export default function AssistantTimeline({ userId = "U1001" }: { userId?: string }) {
  ...
  const loadOnce = React.useCallback(async (signal?: AbortSignal) => {
  ...
  React.useEffect(() => {
    ...
    loadOnce(abortRef.current.signal).catch(e => setErr(e?.message || "Failed to load"));
    start();
    ...
    <div className="rounded-2xl border border-slate-800/60 bg-slate-800/40 backdrop-blur-sm">
      <div className="px-4 py-3 border-b border-slate-800/60 text-sm font-semibold text-slate-200 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href="/member/health-assistant" className="text-xs px-2 py-1 rounded-md bg-sky-600/80 hover:bg-sky-600 text-white">Ask Assistant</a>
          <a href="/member/health-blackbox" className="text-xs px-2 py-1 rounded-md bg-emerald-600/80 hover:bg-emerald-600 text-white">Upload File</a>
          <a href="/member/questionnaires" className="text-xs px-2 py-1 rounded-md bg-amber-600/80 hover:bg-amber-600 text-white">Questionnaires</a>
        </div>
      </div>

      {err && <div className="px-4 py-3 text-sm text-rose-300">{err}</div>}
      {!items && !err && <div className="px-4 py-3 text-sm text-slate-400">Loading…</div>}
      {items && items.length === 0 && <div className="px-4 py-3 text-sm text-slate-400">No assistant activity yet.</div>}

      {items && items.length > 0 && (
        <ul className="px-4 py-2">
          {items.map((it) => (
            <li key={it.id} className="relative pl-6 py-3 border-l border-slate-800/60 last:border-b-0">
              ...
              <div className="text-xs text-slate-400 mt-1">
                ...
                {typeof it.tokensIn === "number" || typeof it.tokensOut === "number" ? (
                  <span> · tokens: {it.tokensIn ?? "-"} / {it.tokensOut ?? "-"}</span>
                ) : null}
              </div>
            </li>
          ))}
