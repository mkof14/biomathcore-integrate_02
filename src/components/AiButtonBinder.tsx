...
function getFab(): HTMLElement | null {
  const sels = [
    '[data-ai-fab]','[data-testid="ai-fab"]','#ai-fab','.ai-fab',
    'button[aria-label*="assistant" i]','button[title*="assistant" i]',
    'button[aria-label*="pulse" i]','button[title*="pulse" i]',
    'a[aria-label*="assistant" i]','a[title*="assistant" i]'
  ];
  for (const s of sels) { const el = document.querySelector<HTMLElement>(s); if (el) return el; }
  const cands = Array.from(document.querySelectorAll<HTMLElement>('button,a,div[role="button"]'))
    .filter(el=>{
      ...
      const rounded = (st.borderRadius && parseFloat(st.borderRadius) >= Math.min(r.width,r.height)*0.4);
