...
function toMB(bytes: number) { return bytes / (1024 * 1024); }
function toGB(bytes: number) { return bytes / (1024 * 1024 * 1024); }
...
export default function UsageCard({
  ...
  const percent = Math.max(0, Math.min(100, Math.round((totalBytes / quotaBytes) * 100)));
  ...
  return (
    <div className={`rounded-xl bg-slate-900/70 border border-slate-800 p-5 ${className}`}>
      ...
      <div className="mb-4 grid grid-cols-2 gap-4 text-sm text-slate-300">
        ...
        <div>
          ...
          <div className="font-medium">{usedGB >= 0.1 ? `${usedGB.toFixed(2)} GB` : `${usedMB.toFixed(0)} MB`}</div>
        </div>
        <div>
          <div className="text-xs text-slate-400">Quota</div>
          <div className="font-medium">{quotaGB >= 0.1 ? `${quotaGB.toFixed(1)} GB` : `${quotaMB.toFixed(0)} MB`}</div>
        </div>
      </div>
      ...
      <div className={`${barHeight} rounded-lg bg-slate-800 border border-slate-700 overflow-hidden relative`} role="progressbar" aria-valuenow={percent} aria-valuemin={0} aria-valuemax={100}>
        <div
        ...
        />
        <div className={`absolute inset-0 flex items-center justify-center ${labelSize} text-slate-100/90`}>
          {percent}%
        </div>
      </div>

      {!hasUsage && <div className="mt-1 text-[11px] text-slate-500">No data yet</div>}
    </div>
  );
}
