    ...
    (async () => {
      try {
        const [s, a] = await Promise.all([
          fetch(`/api/assistant/activity/stats?userId=${userId}`, {
            cache: "no-store",
          }).then((r) => r.json()),
          fetch(`/api/assistant/activity?userId=${userId}`, {
            cache: "no-store",
          }).then((r) => r.json()),
        ]);
        if (!alive) return;
      ...
      <Card title="Quick Actions" subtitle="jump right in">
        <div className="flex flex-wrap gap-2">
          <QLink
            ...
            className="bg-sky-600/80 hover:bg-sky-600"
          >
            Ask Assistant
          </QLink>
          <QLink
...
function Card({
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-800 bg-slate-900/50 p-4 ${className}`}
    >
      <div className="mb-2">
...
function Sparkline({
  ...
  const min = Math.min(...values),
    max = Math.max(...values);
  const xs = values.map(
    (_, i) => pad + i * ((W - 2 * pad) / Math.max(1, values.length - 1)),
  );
  ...
  const d = xs
    .map(
      (x, i) =>
        `${i === 0 ? "M" : "L"}${x.toFixed(1)},${(ys[i] ?? 0).toFixed(1)}`,
    )
    .join(" ");
  const area =
    `M${pad},${H - pad} ` +
    xs.map((x, i) => `L${x.toFixed(1)},${(ys[i] ?? 0).toFixed(1)}`).join(" ") +
    ` L${W - pad},${H - pad} Z`;
  return (
      ...
      <div className="text-xs space-y-1">
        {segs.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            <span
              className={`inline-block w-2.5 h-2.5 rounded-full ${s.cls.replace("text-", "bg-")}`}
            />
...
function QLink({
  ...
  return (
    <Link
      href={href}
      className={`text-xs px-2 py-1 rounded-md text-white ${className}`}
    >
      {children}
    </Link>
  );
