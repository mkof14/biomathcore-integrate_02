...
export async function GET(req: NextRequest) {
  ...
  if (!guard.ok) {
    await auditLogOptional(userId, "report_export_denied", {
      reportId,
      format,
      reason: guard.reason,
    });
  ...
  try {
    const { filename, mime, content } = await exportReport(
      reportId,
      format as "json" | "csv",
    );
    await auditLogOptional(userId, "report_export_ok", {
      reportId,
      format,
      filename,
    });
    ...
    await auditLogOptional(userId, "report_export_error", {
      reportId,
      format,
      error: msg,
    });
