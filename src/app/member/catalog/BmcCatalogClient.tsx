...
export default function BmcCatalogClient({
  categories,
}: {
  categories: Category[];
}) {
  ...
  const addOn = useMemo(
    () =>
      selected.reduce(
        (s, slug) => s + (categories.find((c) => c.slug === slug)?.price ?? 0),
        0,
      ),
    [selected, categories],
  );
      ...
      <header className="mb-6 flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">
            BioMath Core — Service Catalog
          </h1>
          <p className="mt-1 text-sm text-slate-300">
            Core plan <b>${CORE}</b> is always included. Add categories ($4–$7
            each) up to ${CAP}.
          </p>
        </div>
        <button
            ...
            {categories.map((c, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    aria-hidden
                    className=" pointer-events-none inset-0 rounded-2xl ring-1 ring-white/10 -z-10"
                  />
                  ...
                  >
                    <span
                      className={[
                        ...
                        active ? "bg-emerald-400" : "bg-white/70",
                      ].join(" ")}
                    />
                    {active ? "Added" : "Add"}
                  </div>
          ...
          <div className="sticky top-6 rounded-2xl p-5 border border-white/10 bg-slate-900/60 backdrop-blur-md">
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button
                onClick={onContinue}
                type="button"
                className="w-2/3 rounded-xl px-3 py-2 text-sm font-semibold bg-gradient-to-r from-sky-400 to-emerald-400 text-slate-900"
              >
                {capped ? "Go MAX ()" : "Continue"}
              </button>
            </div>
