...
export default function ReportsAllSections() {
  ...
  const fetchReport = async (p: ReportPlan) => {
    ...
    try {
      ...
      (j.sections ?? []).slice(0, 2).forEach((s) => {
        next[s.id] = true;
      });
      setOpen(next);
    } catch (e: any) {
  ...
  const completion = useMemo(() => {
    if (!data) return 0;
    const parts = data.sections.map(
      (s) =>
        (s.items?.length ? 50 : 0) +
        (s.notes?.length ? 20 : 0) +
        (s.charts?.length ? 30 : 0),
    );
  ...
  const kpis = useMemo(() => {
    ...
    const age = flatItems.find((i) =>
      i.label.toLowerCase().includes("biological age"),
    );
    const sleep = flatItems.find((i) =>
      i.label.toLowerCase().includes("sleep last night"),
    );
    const risk = data.sections.find((s) =>
    ...
    return [
      { label: "Plan", value: cap(data.plan), hint: data.title },
      {
        label: "Completion",
        value: `${completion}%`,
        hint: "Sections readiness",
      },
      age
        ? { label: "Bio Age", value: String(age.value), hint: "Estimation" }
        : null,
      sleep
        ? {
            label: "Last Sleep",
            value: String(sleep.value),
            hint: "Last night",
          }
        : null,
      risk
        ? { label: "Risk block", value: "Present", hint: "Insights included" }
        : null,
    ].filter(Boolean) as Array<{ label: string; value: string; hint?: string }>;
  }, [data, completion]);

  const trendPoints = useMemo(() => {
    const c = data?.sections.find((s) =>
      s.charts?.some((ch) => ch.id.includes("hrv")),
    );
    const arr =
      (c?.charts?.find((ch) => ch.id.includes("hrv"))?.data as Array<{
        d: string;
        v: number;
      }>) ?? [];
    ...
    <section className="p-4 sm:p-6 rounded-2xl bg-gradient-to-b from-white/90 to-slate-50 text-slate-800 shadow">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">
            {data?.summary ?? "Human-readable insights with simple language."}
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {kpis.map((k, idx) => (
          <div
            key={idx}
            className="rounded-2xl border bg-gradient-to-b from-white to-blue-50 p-4 shadow-md"
          >
      ...
      >
        ...
        {data?.sections.map((s) => {
          ...
          const secScore =
            (items ? 50 : 0) + (notes ? 20 : 0) + (charts ? 30 : 0);
          const opened = !!open[s.id];

          return (
            <article
              key={s.id}
              className="rounded-2xl border bg-gradient-to-b from-white to-slate-50 p-5 shadow-md"
            >
