  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-slate-500">{new Date(rep.createdAt).toLocaleString()}</div>
      </div>
            ...
            {rep.insights.map((i:any,idx:number)=>(
              <li key={idx} className="flex gap-2">
                <span className={`px-2 py-0.5 rounded text-xs ${
                  i.kind==="risk"?"bg-rose-100 text-rose-700":
                  i.kind==="trend"?"bg-sky-100 text-sky-700":
                  i.kind==="recommendation"?"bg-emerald-100 text-emerald-700":
