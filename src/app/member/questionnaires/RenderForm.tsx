...
export default function RenderForm({
    ...
    if (q.type === "boolean") {
      ...
      return (
        <div className="inline-flex gap-2">
          <button
            type="button"
            className={`px-3 py-1 rounded ${v === true ? "bg-sky-400" : "bg-slate-200"} text-black`}
          ...
          <button
            type="button"
            className={`px-3 py-1 rounded ${v === false ? "bg-sky-400" : "bg-slate-200"} text-black`}
  ...
  const handleSubmit = async () => {
    ...
    const res = await submitQuestionnaire(slug, {
      answers: { ...answers, ...meta },
    });
    if (res.ok) {
      const next = nextSlug(slug);
      if (next) router.push(`/member/questionnaires/${next}`);
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className="font-semibold">
                {anonymous ? `User ID ${getUserId()}` : "Name on file"}
              </span>
            </div>
            <button
              type="button"
              className={`px-3 py-1 rounded ${anonymous ? "bg-sky-400" : "bg-slate-200"} text-black`}
      ...
      <div className="w-full bg-slate-200 rounded h-3">
        <div
          className="bg-gradient-to-r from-sky-400 to-emerald-400 h-3 rounded"
      ...
      <div className="flex gap-4 items-center">
        <span>Units:</span>
        <button
          className={`px-3 py-1 rounded ${"metric" === metricSystem ? "bg-sky-400" : "bg-slate-200"} text-black`}
          onClick={() => setMetricSystem("metric")}
        >
          Metric
        </button>
        <button
          className={`px-3 py-1 rounded ${"imperial" === metricSystem ? "bg-sky-400" : "bg-slate-200"} text-black`}
      ...
      <div className="flex justify-between pt-4">
        <button
          className="px-6 py-3 bg-gradient-to-r from-sky-400 to-emerald-400 text-black font-semibold rounded"
