...
export default function Page() {
  const [env, setEnv] = useState<{ data?: EnvMap; stripeMode?: string } | null>(
    null,
  );
  ...
  async function load() {
    try {
      ...
      const [envJ, hJ, aJ] = await Promise.all([
        envR.json(),
        hR.json(),
        aR.json(),
      ]);
  ...
  const checks = useMemo(() => {
    ...
    const endpointsOk = health
      ? Object.values(health).every((x) => x.ok)
      : false;
    ...
    return [
      ...
      {
        key: "stripeLive",
        label: "Stripe in LIVE mode (optional)",
        ok: stripeLive,
      },
      {
        key: "dbConnected",
        label: "Production database URL configured",
        ok: dbConnected,
      },
      {
        key: "endpointsOk",
        label: "All health endpoints are OK",
        ok: endpointsOk,
      },
      {
        key: "domainConfigured",
        label: "Domain configured (NEXT_PUBLIC_APP_URL)",
        ok: domainConfigured,
      },
    ];
  }, [env, health]);
  ...
  return (
    <div className="p-6 space-y-6">
      <SectionHeader
        title="Release"
        desc="Pre-ship checklist. Auto-updates every 30s."
      />
