  ...
  return (
    <div className="space-y-6">
      <SectionCard title="Environment Health" descr="   ">
        {!env ? (
          <div className="kicker">Checking…</div>
        ) : (
          <>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="admin-card p-3">
                <div className="font-medium mb-2">
                  Present ({env.present.length}/{env.total})
                </div>
                <ul className="text-sm list-disc pl-5 space-y-1">
                  {env.present.map((k) => (
                    <li key={k} className="tag tag-ok">
                      {k}
                    </li>
                  ))}
                </ul>
              ...
              <div className="admin-card p-3">
                <div className="font-medium mb-2">
                  Missing ({env.missing.length}/{env.total})
                </div>
                <ul className="text-sm list-disc pl-5 space-y-1">
                  {env.missing.length ? (
                    env.missing.map((k) => (
                      <li key={k} className="tag tag-bad">
                        {k}
                      </li>
                    ))
                  ) : (
            ...
            <div className="admin-card p-3 mt-4">
                ...
                <button
                  ...
                  onClick={async () => {
                    const res = await fetch(
                      `/api/admin/secrets/export?filename=${encodeURIComponent(envFilename)}`,
                    );
                    const blob = await res.blob();
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement("a");
                    a.href = url;
                    a.download = envFilename || ".env.local";
              ...
              <div className="small mt-2">
                offline-:{" "}
                <code>node scripts/env-from-secrets.js .env.local</code>
              </div>
            </div>
          </>
        )}
