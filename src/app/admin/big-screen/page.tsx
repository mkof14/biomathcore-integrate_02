...
export default function BigScreen() {
  ...
  const load = React.useCallback(async () => {
    try {
      const [s, d30, h, ds] = await Promise.all([
        fetch("/api/analytics/summary", { cache: "no-store" }).then((r) =>
          r.ok ? r.json() : null,
        ),
        fetch("/api/analytics/by-day?days=30", { cache: "no-store" }).then(
          (r) => (r.ok ? r.json() : null),
        ),
        fetch("/api/health/all", { cache: "no-store" }).then((r) =>
          r.ok ? r.json() : null,
        ),
        fetch("/api/dashboard/summary", { cache: "no-store" }).then((r) =>
          r.ok ? r.json() : null,
        ),
      ]);
  ...
  const trafficSpark = byDay.map((p) => ({
    date: p.date,
    value: p.total ?? 0,
  }));
    ...
    <div className="mx-auto max-w-[1800px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <BigStat
          ...
          sub="AI + Voice + Drug–Gene"
        />
        <BigStat
          label="Today (est.)"
          value={lastTotal.toLocaleString()}
          sub="Requests today (from last bucket)"
        />
        <BigStat
          label="Active Services"
          value={`${okCount}`}
          sub={`${errCount} degraded`}
          accent="emerald"
        />
        <BigStat
          label="MRR"
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-950/60 p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <BigStat
              ...
              value={
                revenueMTD != null
                  ? revenueMTD.toLocaleString(undefined, {
                      style: "currency",
                      currency: "USD",
                    })
                  : "—"
          ...
          <div className="mt-3 text-xs text-neutral-400">
            Wire Stripe metrics to populate any “—”.
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs text-neutral-400">
                  {v?.ok ? "OK" : v?.status ? `ERR ${v.status}` : "ERR"}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="mt-6 rounded-xl border border-neutral-800 bg-neutral-950/60 p-3">
        ...
        <div className="text-xs text-neutral-300 whitespace-nowrap overflow-hidden">
          <div className="animate-[scroll_60s_linear_infinite] inline-block will-change-transform">
            ...
            <span className="mr-8">
              Connect cohort API to enable retention analytics.
            </span>
            <span className="mr-8">
              Error budget: wire SLO data to display.
            </span>
          </div>
        </div>
        <style jsx>{`
      ...
      <div className="py-6 text-center text-[11px] text-neutral-500">
        Full-screen safe · Muted palette · Labels include full names under
        abbreviations
      </div>
    </div>
  );
