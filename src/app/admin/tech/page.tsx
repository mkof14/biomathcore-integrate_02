...
const GLOSSARY: Array<{ abbr: string; full: string; desc: string }> = [
  {
    ...
    desc: "Targeted uptime and response guarantees.",
  },
  {
    abbr: "SLO",
    full: "Service Level Objective",
    desc: "Measured objectives (e.g., 99.9% uptime, p95 latency).",
  },
  {
    abbr: "SLA Breach",
    full: "Agreement Breach",
    desc: "When actual performance falls below SLA commitments.",
  },
  {
    abbr: "RTO",
    full: "Recovery Time Objective",
    desc: "Max acceptable outage duration.",
  },
  {
    abbr: "RPO",
    full: "Recovery Point Objective",
    desc: "Max acceptable data loss window.",
  },
  {
    abbr: "p95 Latency",
    full: "95th Percentile Latency",
    desc: "Latency under which 95% of requests complete.",
  },
];
...
export default function Page() {
  ...
  React.useEffect(() => {
    (async () => {
      const h = await fetch("/api/health/all", { cache: "no-store" }).then(
        (r) => (r.ok ? r.json() : null),
      );
      setHealth(h?.data ?? h ?? {});
    })();
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <div className="text-neutral-400">
          Endpoints, uptime language, and operational glossary.
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <BigStat
          label="Error Budget"
          value="—"
          sub="Wire your SLOs to populate"
        />
      </div>
          ...
          {GLOSSARY.map((g) => (
            ...
            >
              <div className="text-sm font-medium">
                {g.full} <span className="text-neutral-400">({g.abbr})</span>
              </div>
