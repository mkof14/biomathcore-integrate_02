...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const d = await fetch("/api/analytics/by-day?days=30", {
        cache: "no-store",
      }).then((r) => r.json());
      setSummary({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card tone="violet">
          <CardHeader>
            <CardTitle>AI (30d)</CardTitle>
          </CardHeader>
          <CardBody>
            <div className="text-4xl font-bold tabular-nums">{summary.ai}</div>
          </CardBody>
        </Card>
        <Card tone="emerald">
          <CardHeader>
            <CardTitle>Voice (30d)</CardTitle>
          </CardHeader>
          <CardBody>
        ...
        <Card tone="amber">
          <CardHeader>
            <CardTitle>Drug–Gene (30d)</CardTitle>
          </CardHeader>
          <CardBody>
            <div className="text-4xl font-bold tabular-nums">{summary.dg}</div>
          </CardBody>
        </Card>
      </div>
      <Card tone="slate">
