...
export default function Finance() {
  ...
  const exportCSV = () => {
    if (!data) return;
    const rows = [["Type", "Label", "Revenue"]].concat(
      ["daily", "monthly", "yearly"].flatMap((k: any) =>
        (data[k] || []).map((r: any) => [k, r.d ?? r.m ?? r.y, r.revenue]),
      ),
    );
  ...
  const cohorts = useMemo(
    () =>
      Array.from({ length: 6 }).map((_, i) => ({
        cohort: `2025-${i + 1}`,
        repeats: Math.round(30 + Math.random() * 60),
      })),
    [],
  );
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {(["daily", "monthly", "yearly"] as const).map((k) => (
          <button
            key={k}
            onClick={() => setMode(k)}
            className={`px-3 py-1 rounded ${mode === k ? "bg-sky-500 text-slate-900" : "bg-slate-800"}`}
          >
            {k}
          </button>
        ))}
        <button
          onClick={exportCSV}
          className="ml-auto px-3 py-1 rounded bg-emerald-400 text-slate-900 font-semibold"
        >
          Export CSV
        </button>
      </div>
      ...
      <div className="bg-[#0c1324] border border-slate-800 rounded-xl p-4">
        ...
        {current.length > 0 && (
          <Bar
            data={current}
            labelKey={mode === "daily" ? "d" : mode === "monthly" ? "m" : "y"}
            valueKey="revenue"
          />
        )}
      </div>
