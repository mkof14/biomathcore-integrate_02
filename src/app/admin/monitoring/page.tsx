      ...
      setServices((prev) =>
        prev.map((s) => {
          const jitter = Math.max(
            40,
            Math.min(400, Math.round(s.latencyMs + (Math.random() * 20 - 10))),
          );
        ...
        <Card title="Uptime (sample)">
          <div className="h-[220px] grid grid-cols-12 gap-1">
            {Array.from({ length: 96 }).map((_, i) => (
              <div
                key={i}
                className={`h-4 ${i % 13 === 0 ? "bg-rose-500/60" : "bg-emerald-500/60"}`}
              />
            ))}
          </div>
        </Card>
