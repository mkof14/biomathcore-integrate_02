...
export default function DemoDG() {
  ...
  async function run() {
    const res = await fetch("/api/drug-gene/query", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ gene, drug }),
    });
  ...
  return (
    <div className="p-6 space-y-4">
      <ActionBar
        title="Demo — Drug–Gene"
        extra={<EndpointBadge path="/api/drug-gene/query" />}
      />
      <div className="max-w-2xl space-y-2">
        ...
        {!!out && (
          <pre className="p-3 rounded border border-neutral-800 bg-neutral-900/40 whitespace-pre-wrap">
            {out}
          </pre>
        )}
      </div>
