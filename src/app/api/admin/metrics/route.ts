...
export async function GET() {
  try {
    const [summary, byDay, health] = await Promise.all([
      jsonGet("/api/analytics/summary").catch(() => ({ data: { /* TODO: implement or remove */ } })),
      jsonGet("/api/analytics/by-day?days=30").catch(() => ({ data: [] })),
      jsonGet("/api/health/all").catch(() => ({ data: { /* TODO: implement or remove */ } })),
    ]);

    const endpoints = (health as unknown)?.data ?? { /* TODO: implement or remove */ };
    const endpointsTotal = Object.keys(endpoints).length;
    const endpointsOk = Object.values(endpoints).filter((x: unknown) => x?.ok).length;
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        analytics: summary?.data ?? { /* TODO: implement or remove */ },
