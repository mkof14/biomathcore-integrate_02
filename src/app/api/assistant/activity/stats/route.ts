...
export async function GET(req: Request) {
  ...
  Object.values(hourBuckets).forEach((b) => {
    b.avgMs = b.total ? Math.round(b.sumMs / b.total) : 0;
  });
  const last24 = Object.entries(hourBuckets).map(([k, b]) => ({
    hour: k,
    total: b.total,
    ok: b.ok,
    fallback: b.fallback,
  ...
  const last7 = Object.entries(dayBuckets).map(([k, b]) => ({
    day: k,
    total: b.total,
    ok: b.ok,
    fallback: b.fallback,
  ...
  const fallbackPct = totals
    ? Math.round(
        (1000 * rows.filter((r) => r.status === "fallback").length) / totals,
      ) / 10
    : 0;
  const avgMs = rows.length
    ? Math.round(
        rows.reduce((s: any, r: any) => s + (r.duration ?? 0), 0) / rows.length,
      )
    : 0;
  ...
  return NextResponse.json({
    last24,
    last7,
    metrics: { total: totals, fallbackPct, avgMs },
  });
}
