...
const BASE: Field[] = [
  ...
  {
    name: "consent",
    label: "I consent to store and process my data for care",
    type: "checkbox",
    required: true,
  },
];
  ...
  "nutrition-diet": [
    {
      ...
      label: "Diet style",
      type: "select",
      options: ["Balanced", "Keto", "Vegan", "Other"],
    },
  ],
};
...
export async function POST(req: Request) {
  ...
  const categories = Array.isArray((body as unknown).categories)
    ? ((body as unknown).categories as string[])
    : [];
  ...
  return NextResponse.json({
    questionnaireId: "dynamic-intake",
    fields,
    categories,
  });
}
