...
): Promise<{ data: AIRow[]; nextCursor?: string }> {
...
export async function countAIRuns(): Promise<number> {
  return mem().size;
}
export async function seedAIRuns(n = 5): Promise<number> {
  for (let i = 0; i < n; i++) {
    await createAIRun({
      title: `Demo run #${i + 1}`,
      status: (i % 2 === 0 ? "completed" : "queued") as unknown,
    });
  }
  return mem().size;
}
export async function resetAIRuns(): Promise<void> {
  mem().clear();
}
