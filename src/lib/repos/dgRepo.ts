...
function toArr() {
  return Array.from(g.__DG__.values()) as Base[];
}
...
export async function createDG(partial: Partial<Base>): Promise<Base> {
  ...
  const row: Base = {
    id: uuid(),
    title: partial.title || "Untitled DG",
    status: (partial.status as unknown) || "draft",
    content: partial.content || "",
    createdAt: now,
    updatedAt: now,
  };
  g.__DG__.set(row.id, row);
  return row;
}
...
export async function getDG(id: string) {
  return g.__DG__.get(id) as Base | undefined;
}
export async function updateDG(id: string, patch: Partial<Base>) {
  const r = await getDG(id);
  if (!r) throw new Error("not_found");
  Object.assign(r, patch, { updatedAt: new Date() });
  g.__DG__.set(id, r);
  return r;
}
export async function deleteDG(id: string) {
  const r = await getDG(id);
  if (!r) throw new Error("not_found");
  g.__DG__.delete(id);
  return r;
}
export async function countDG() {
  return g.__DG__.size as number;
}
