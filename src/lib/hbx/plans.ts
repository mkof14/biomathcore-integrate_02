...
export function getUserPlan(userId: string): PlanId { return g.__HBX_USER_PLANS.get(userId) ?? "free"; }
export function setUserPlan(userId: string, plan: PlanId) { g.__HBX_USER_PLANS.set(userId, plan); }
export function getLimits(userId: string): Limits { return PLANS[getUserPlan(userId)]; }
