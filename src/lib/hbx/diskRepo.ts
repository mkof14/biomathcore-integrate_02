...
function dirForUser(userId: string) {
  return path.join(ROOT, userId);
}
function filePaths(userId: string, id: string) {
  const dir = dirForUser(userId);
  return {
    enc: path.join(dir, `${id}.enc`),
    meta: path.join(dir, `${id}.meta.json`),
  };
}
export async function ensureDirs(userId: string) {
  await fsp.mkdir(dirForUser(userId), { recursive: true });
}
export async function saveEncrypted(opts: {
  userId: string;
  id: string;
  filename: string;
  mime: string;
...
export async function listMetas(userId: string, limit = 200): Promise<Meta[]> {
  try {
    ...
    for (const m of metas)
      out.push(JSON.parse(await fsp.readFile(path.join(dir, m), "utf8")));
...
export async function streamDecrypted(
  userId: string,
  id: string,
...
export async function usage(userId: string): Promise<Usage> {
  ...
  let bytes = 0;
  for (const m of metas) bytes += m.size;
