...
export function buildFallbackReply(
  ...
  lines.push(
    lang.startsWith("ru")
      ? "Краткое резюме по вашим данным:"
      : "Brief summary from your data:",
  );
  ...
  const profileBits = [
    ...
    age != null
      ? lang.startsWith("ru")
        ...
        : `Age: ${age}`
      : null,
    ctx.user.conditions?.length
      ? lang.startsWith("ru")
        ? `Состояния: ${ctx.user.conditions.join(", ")}`
        : `Conditions: ${ctx.user.conditions.join(", ")}`
      : null,
    ctx.user.medications?.length
      ? lang.startsWith("ru")
        ? `Препараты: ${ctx.user.medications.join(", ")}`
        : `Meds: ${ctx.user.medications.join(", ")}`
      : null,
    ctx.user.allergies?.length
      ? lang.startsWith("ru")
        ? `Аллергии: ${ctx.user.allergies.join(", ")}`
        : `Allergies: ${ctx.user.allergies.join(", ")}`
      : null,
  ].filter(Boolean);
  ...
  if (facts.length) {
    ...
    facts.slice(0, 6).forEach((f, i) => {
      ...
      lines.push(
        `  [${i + 1}] (${label} ${f.sourceId}) ${f.text.slice(0, 240)}`,
      );
    });
  }
  ...
  const tips: string[] = [];
  const textAll =
    facts.map((f) => f.text.toLowerCase()).join(" ") +
    " " +
    (JSON.stringify(ctx.reports) || "").toLowerCase();
  const mentions = (s: string) => textAll.includes(s);

  if (
    mentions("sleep") ||
    mentions("сон") ||
    JSON.stringify(ctx.forms).toLowerCase().includes("sleep")
  ) {
    tips.push(
      lang.startsWith("ru")
